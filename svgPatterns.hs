...
svgEstrela ((x1,y1),(x2,y2),(x3,y3),(x4,y4),(x5,y5)) style =
  printf "<polygon points='%.3f,%.3f %.3f,%.3f %.3f,%.3f %.3f,%.3f %.3f,%.3f' style='%s'/>\n" x1 y1 x2 y2 x3 y3 x4 y4 x5 y5 style
...
main = do
  ...
  where svgstrs = svgBegin w h ++ svgRetangulo ++ svgTwoTri ++ svgCirculo ++ svgTriangulo ++ svgStar ++ svgEnd
        ...
        svgTriangulo = svgElements svgTriangle triangulo (map svgStyleTriangle corTriangle)
        triangulo = genTriangleInLine ntriangle
        corTriangle = rgbTriangulo ntriangle
        ntriangle = 1

        svgTwoTri = svgElements svgTriangle triangle (map svgStyleTriangle corTriangulo)
        triangle = genTrianguloInLine ntri
        corTriangulo = rgbTriangulo ntri
        ntri = 1
        ...
        (w,h) = (800,500) -- width,height da imagem SVG 
